...
class ChatSystem {
  ...
  processJobQuery(query) {
    const jobKeywords = ['job', 'jobs', 'vacancy', 'vacancies', 'career', 'hiring', 'opening'];
    const hasJobKeyword = jobKeywords.some(keyword => query.includes(keyword));

    if (!hasJobKeyword) return null;

    const intent = this.detectJobIntent(query);
    const context = this.extractJobContext(query);
    const relevantJobs = this.getRelevantJobs(intent, context);

    if (relevantJobs.length > 0) {
      return this.generateJobResponse(relevantJobs, intent, context);
    } else {
      return "Sorry, I couldn't find any jobs matching your request. Please try a different query.";
    }
  }

  detectJobIntent(query) {
    const intents = {
      'software': ['software', 'developer', 'programmer', 'engineer'],
      'data': ['data', 'scientist', 'analyst'],
      'banking': ['bank', 'banking', 'finance'],
      'government': ['police', 'government', 'public'],
  ...
  extractJobContext(query) {
    const context = {
      location: this.extractLocation(query),
      salary: this.extractSalary(query),
      experience: this.extractExperience(query),
      company: this.extractCompany(query),
      type: this.extractJobType(query)
    };
  ...
  extractLocation(query) {
    const locations = ['bangalore', 'bhopal', 'hyderabad', 'mumbai', 'madhya pradesh'];
    return locations.find(location => query.includes(location)) || null;
  }

  extractSalary(query) {
    const salaryMatch = query.match(/(\d+,\d+|\d+)\s*(rs|inr|₹)/i);
    return salaryMatch ? parseFloat(salaryMatch[1].replace(/,/g, '')) : null;
  }

  extractExperience(query) {
    const experienceMatch = query.match(/(\d+)\s*-\s*(\d+)\s*years/i);
    return experienceMatch ? { min: parseInt(experienceMatch[1]), max: parseInt(experienceMatch[2]) } : null;
  }

  extractCompany(query) {
    const companies = ['google', 'tech corp', 'data insights', 'national bank', 'madhya pradesh police'];
    return companies.find(company => query.includes(company)) || null;
  }

  extractJobType(query) {
    const types = ['full-time', 'part-time', 'contract', 'internship', 'government'];
    return types.find(type => query.includes(type)) || null;
  }

  getRelevantJobs(intent, context) {
    ...
    if (intent !== 'general') {
      jobs = jobs.filter(job => {
        ...
        return title.includes(intent) || description.includes(intent);
      });
    }
    ...
    if (context.salary) {
      jobs = jobs.filter(job => {
        ...
        return jobSalary >= context.salary;
      });
    }
    ...
    if (context.experience) {
      jobs = jobs.filter(job => {
        ...
        if (experienceMatch) {
          ...
          return context.experience.min >= minExp && context.experience.max <= maxExp;
        }
        return false;
      });
    }
  ...
  generateJobResponse(jobs, intent, context) {
    if (jobs.length === 0) {
      return "Sorry, I couldn't find any jobs matching your request.";
    }

    const header = this.generateJobHeader(intent, context);
  ...
  generateJobHeader(intent, context) {
    ...
    if (context.salary) {
      headerText += ` with salary above ₹${context.salary.toLocaleString()}`;
    }
